...
export const Button: React.FC<ButtonProps> = ({
  ...
  const getBackgroundColor = (): string => {
    if (isDisabled && (variant === 'solid' || variant === 'elevated')) return colors.disabled;
  ...
  const getBorderStyle = () => {
    if (variant === 'outline') {
      return { borderWidth: 1.5, borderColor: isDisabled ? colors.disabled : resolvedColor };
    }
    return {};
  };
  ...
  const getShadowStyle = () => {
    if (variant === 'elevated' && !isDisabled) {
      return { shadowColor: resolvedColor, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8, elevation: 4 };
    }
    return {};
  };
  ...
  const getTextColor = (): string => {
    ...
    if (variant === 'solid' || variant === 'elevated' || variant === 'destructive') return colors.textInverse;
    return resolvedColor;
  };
      ...
      {loading ? (
        ...
        <View style={styles.content}>
          ...
          <Text
            style={[
              styles.label,
              { fontSize: sizeConfig.fontSize, color: getTextColor(), fontWeight: '600' },
